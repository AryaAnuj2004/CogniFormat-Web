...
# ==================== MAIN SECTION: DOWNLOAD GATE & PREVIEW ====================
dl_col, prev_col = st.columns([1.1, 1])

with dl_col:
    st.markdown("""
    <div class="download-card">
        <div class="card-title">Download CogniFormat for Windows</div>
    ...
    if st.session_state.unlocked:
        st.markdown('<div class="notice-unlocked">Authorized email verified! Download your 1-click Windows installer below.</div>', unsafe_allow_html=True)
        ...
        if os.path.exists(EXE_PATH):
            ...
            st.link_button("Download CogniFormat_Setup.exe (v1.0.0)", SETUP_EXE_URL)
        else:
            st.error("Installer executable file missing from assets directory.")
    else:
        st.markdown('<div class="notice-locked">Enter an authorized email address above to activate your free download link.</div>', unsafe_allow_html=True)

    st.markdown("</div>", unsafe_allow_html=True)

with prev_col:
    st.markdown("### Visual App Workspace")
    dash_path = os.path.join(SCREENSHOTS_DIR, "dashboard.jpg")
    batch_path = os.path.join(SCREENSHOTS_DIR, "batch.jpg")
    
    prev_tab1, prev_tab2 = st.tabs(["Main Workspace", "Batch Processing"])
    
    with prev_tab1:
        if os.path.exists(dash_path):
            st.image(dash_path, caption="CogniFormat Studio Workspace & Diff Viewer", use_container_width=True)
        else:
            st.info("Workspace screenshot preview")

    with prev_tab2:
        if os.path.exists(batch_path):
            st.image(batch_path, caption="High-Speed Batch Processing Queue Manager", use_container_width=True)
        else:
            st.info("Batch queue manager preview")
